...
// реализовать FileRaiiWrapper для управления FILE* по парадигме RAII
    try {
        FileRaiiWrapper fwr ("test_read.txt");
        assert(fwr.ReadBytes(10) == "some text");
        std::cout << fwr.ReadBytes(10) << std::endl;
...
// выделение памяти так и должно остаться в куче в тех местах, где оно уже есть
    std::vector<std::unique_ptr<FILE, int (*)(FILE*)>> vector_files;
    FILE* tmpfile = std::fopen("test_write.txt", "w");
    vector_files.emplace_back(tmpfile, &std::fclose);
    std::unique_ptr<char[]> data = std::make_unique<char[]>(11);
    std::strcpy(data.get(), "hello text");
    std::fputs(data.get(), vector_files[0].get());
